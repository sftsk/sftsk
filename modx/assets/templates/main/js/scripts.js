...
$(document).ready(function() {		
	...
	$(window).bind( "hashchange", function(e) {
		...
		if (url)
		{
			$('#content').load("/modx/"+url+ ".html #single_project", projectPreloader);				  
		} 
	}); 
...
function showProject(proj_obj) {
	proj_obj.fadeIn(100, function() {
        // Animation complete
        if(proj_obj.next().hasClass('project')){
        	showProject(proj_obj.next());
        }
      });	
}
...
function projectLoaded() {		
	...
	$('video,audio').mediaelementplayer({flashName: 'flashmediaelement.swf'});
