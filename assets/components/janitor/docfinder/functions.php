<?php 
						...
						{ 
							?>
								...
								<div class="tab-page" id="tab<?php echo $id; ?>">
									<h2 class="tab"><?php echo $name; ?> (<span id="hits<?php echo $id; ?>"></span>)</h2>									
	...
	function printResultTables($area, $search, $searchOptions, $theme, $results, $searchPlacesArray)
	{
		...
		// define action IDs
		$actionIDsArray['DocAndTVV']['edit']=30;
		$actionIDsArray['DocAndTVV']['info']=28;
		$actionIDsArray['Templates']['edit']=19;
		$actionIDsArray['TVs']['edit']=23;
		$actionIDsArray['Chunks']['edit']=10;
		$actionIDsArray['Snippets']['edit']=16;
		$actionIDsArray['Plugins']['edit']=13;
		...
		switch($area)
		{
			...
			case "DocAndTVV":
						...
						<table class="DocAndTVV" id="<?php echo $area; ?>">
							<thead>
								<tr>
									...
									<th class="title <?php echo $sortableClass; ?>" axis="string">Pagetitle</th>
									<th class="title <?php echo $sortableClass; ?>" axis="string">Longtitle</th>
									<th class="title <?php echo $sortableClass; ?>" axis="date">Created on</th>
									<th class="title <?php echo $sortableClass; ?>" axis="date">Edited on</th>
									<?php  if($search['string']!="ALL") { ?>
									<th class="found_in <?php echo $sortableClass; ?>" axis="string">Found in</th>
									<?php  } ?>
				...
				{
					// set URLs
					$urlEdit=$modxConfig['site_url'] . "manager/index.php?a=".$editActionID."&amp;id=".$searchResults['id'];
					$urlInfo=$modxConfig['site_url'] . "manager/index.php?a=".$infoActionID."&amp;id=".$searchResults['id'];
					...
					?>
						<tr>
							...
							<?php  if($search['string']!="ALL") { ?>
							<td class="found_in"><?php echo substr($searchResults['found_in'], 2); ?></td>
							<?php  } ?>
							<td class="functions">
								<a href="<?php echo $urlEdit; ?>" title="Edit" target="_blank"><img src="media/style<?php echo $theme; ?>/images/icons/save.png" alt="Edit" /></a>
								<a href="<?php echo $urlInfo; ?>" title="Info" target="_blank"><img src="media/style<?php echo $theme ;?>/images/tree/page-html.gif" alt="Info" /></a>
								<a href="<?php echo $urlOpen; ?>" title="Preview" target="_blank"><img src="media/style<?php echo $theme; ?>/images/icons/page_white_magnify.png" alt="Preview" /></a>
							</td>
						</tr>
					<?php 
						...
						<table class="Resources" id="<?php echo $area; ?>_table">
							<thead>
								<tr>
									...
									<th class="name <?php echo $sortableClass; ?>" axis="string">Name</th>
									<th class="description <?php echo $sortableClass; ?>" axis="string">Description</th>
									<?php  if($search['string']!="ALL") { ?>
									<th class="found_in <?php echo $sortableClass; ?>" axis="string">Found in</th>
									<?php  } ?>
									<th class="functions">Functions</th>
					...
					<table class="Resources" id="<?php echo $area; ?>">
						<thead>
							<tr>
								<th class="no align_right <?php echo $sortableClass; ?>" axis="number">#</th>
								<th class="align_right id <?php echo $sortableClass; ?>" axis="number">ID</th>
								<th class="name <?php echo $sortableClass; ?>" axis="string">Type</th>
								<th class="name <?php echo $sortableClass; ?>" axis="string">Pagetitle / Name</th>
								<th class="description <?php echo $sortableClass; ?>" axis="string">Longtitle / Description</th>
								<th class="title <?php echo $sortableClass; ?>" axis="date">Created on</th>
								<th class="title <?php echo $sortableClass; ?>" axis="date">Edited on</th>
								<?php  if($search['string']!="ALL") { ?>
								<th class="found_in <?php echo $sortableClass; ?>" axis="string">Found in</th>
	...
	function printHistory($type)
	{
		...
		if($_SESSION[$type.'_history'])
		{
			...
			// output
			?>
				<option value=""><?php echo ucfirst($type); ?> history ...</option>
			<?php 
		...
		{
			?>
				<option value=""><?php echo ucfirst($type); ?> history (empty)</option>
			<?php 
		}
